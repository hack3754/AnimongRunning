...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private void SetPagePositions() {
        ...
        if (_horizontal) {
            ...
            _fastSwipeThresholdMaxLimit = (int)_scrollRectRect.rect.width;
        } else {
        ...
        for (int i = 0; i < _pageCount; i++) {
            ...
            if (_horizontal) {
                childPosition = new Vector2(i * width - containerWidth / 2 + offsetX, m_Interval.y);
            } else {
                childPosition = new Vector2(m_Interval.x, -(i * height - containerHeight / 2 + offsetY));
            }
